...
var AudioPlayer = class extends import_obsidian4.Plugin {
  async onload() {
    ...
    // Добавлена команда "Start" для принудительного воспроизведения
    this.addCommand({
      id: "start-audio",
      name: "Start Audio",
      callback: () => {
        ...
        if (!activeFile) {
          new import_obsidian4.Notice("No active note to get default audio");
          return;
        }

        // Формируем имя файла по умолчанию: title.mp3
        const defaultFilename = `${activeFile.basename}.mp3`;
        
        // Ищем файл по умолчанию
        const defaultFile = this.app.metadataCache.getFirstLinkpathDest(
          defaultFilename,
          activeFile.path
        );

        if (!defaultFile) {
          new import_obsidian4.Notice(`Default audio not found: ${defaultFilename}`);
          return;
        }

        // Устанавливаем и воспроизводим файл по умолчанию
        player.src = this.app.vault.getResourcePath(defaultFile);
        player.play();
        new import_obsidian4.Notice(`Playing audio: ${defaultFilename}`);
      }
    });
	    this.addCommand({
        id: "stop-audio",
        ...
        callback: () => {
            ...
            // Удалить источник
            player.src = "";
            // Уведомить компоненты о сбросе
            document.dispatchEvent(new Event('allpause'));
            document.dispatchEvent(new Event('stop'));
            new import_obsidian4.Notice("Audio stopped");
        }
    });
